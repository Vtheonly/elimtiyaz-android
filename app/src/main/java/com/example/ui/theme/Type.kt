...
/**
 * Material 3 typography scale — complete set, replacing the previous
 * stub that only overrode `bodyLarge`.
 *
 * Font family: System default (San Francisco on Android 12+ via Roboto
 * Flex fallback). The desktop uses Inter + Noto Sans Arabic; we use the
 * system default to respect user font preferences and reduce APK size.
 *
 * For Arabic RTL support, the system handles font fallback automatically.
 */
